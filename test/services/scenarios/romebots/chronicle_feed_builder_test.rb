...
module Scenarios
  module Romebots
    class ChronicleFeedBuilderTest < ActiveSupport::TestCase
      setup do
        reset_game_data!
      ...
      test "chronicle feed builder returns readable entries in chronological order" do
        ...
        entries = ChronicleFeedBuilder.new(game_session: session).entries

        assert_equal ["session_started", "year_started", "response_resolved"], entries.first(3).map(&:event_type)
        assert_equal entries.sort_by(&:occurred_at).map(&:occurred_at), entries.map(&:occurred_at)
      end
      ...
      test "session state changes can appear alongside the related narrative event" do
        ...
        Choices::ResolveResponse.call(game_session: session, response_key: "a")
        entry = ChronicleFeedBuilder.new(game_session: session.reload).entries.find(&:primary?)
